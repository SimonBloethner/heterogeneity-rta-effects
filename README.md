...
The raw input is committed under `archive/retired_pack/` and must be copied to
the path the pipeline expects before Stage 1 runs. See `docs/DATA.md`.
...
commit `4325590`, enforce.R reports zero violations.
...
- R 4.4.1 on x86_64 Linux
- `data.table` 1.15.4, `dplyr` 1.1.4, `fixest` 0.13.2, `tidyr` 1.3.1,
  `reshape2` 1.4.4, `ragg` 1.3.3, `POT` 1.1-11
- Heavy stages are written for SLURM compute nodes; several scripts assert
  they are not running on a login node.

`meta/ENVIRONMENT.txt` records the environment that produced the canonical
numbers, with per-package version, build string and library path. It is
generated by `code/S48_environment.R`. Every package build predates the
canonical runs of 2026-07-26 to 2026-08-06, so the record describes the
environment those numbers came from rather than a later one.

Two versions are worth attention. `fixest` and `POT` resolve from a user
library rather than the system R tree, and `fixest` is pinned at 0.13.2 exactly
rather than at a floor. PPML fixed-effect convergence is not guaranteed to be
identical across `fixest` builds, so check a run against the validation gate
table above before concluding that any difference is substantive.
